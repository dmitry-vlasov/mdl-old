...
/* File Name:       nstd_interpolation_Gradient_2.cpp                        */
...
#ifndef NSTD_INTERPOLATION_GRADIENT_2_CPP_
#define NSTD_INTERPOLATION_GRADIENT_2_CPP_
...
namespace interpolation {
	...
	degreeX_ (degreeX),
	degreeY_ (degreeY),
	dim_ ((degreeX_ + 1) * (degreeY_ + 1)),
	sample_ (sample),
	a_zero_ (degreeX_, degreeY_, sample_),
	a_plus_ (degreeX_, degreeY_, sample_),
	a_minus_ (degreeX_, degreeY_, sample_),
	a_prime_ (degreeX_, degreeY_, sample_),
	epsilon_ (0.0001),
	count_0_ (0),
	count_1_ (0),
	count_2_ (0),
	...
	template<class A>
	typename Gradient<2, A> :: Polynomial_&
	Gradient<2, A> :: evaluate (const Real epsilon)
	{
		if (epsilon != 0) {
			epsilon_ = epsilon;
		}
	...
	template<class A>
	inline const typename Gradient<2, A> ::Approximation_&
	Gradient<2, A> :: getApproximation() const {
		return a_zero_;
	}

	template<class A>
	inline Integer
	Gradient<2, A> :: iterationsRavine() const {
		return count_0_;
	}
	template<class A>
	inline Integer
	Gradient<2, A> :: iterationsFirst() const {
		return count_1_;
	}
	template<class A>
	inline Integer
	Gradient<2, A> :: iterationsSecond() const {
		return count_2_;
	}
...
using manipulator :: tab;
	...
	template<class S>
	inline void
	Gradient<2, A> :: show (S& string) const {
		a_zero_.show (string);
	}
	...
	template<class A>
	void
	Gradient<2, A> :: commitSuicide() {
		delete this;
	}
	template<class A>
	Size_t
	Gradient<2, A> :: getVolume() const
	{
		Size_t volume = 0;
	...
	template<class A>
	Size_t
	Gradient<2, A> :: getSizeOf() const {
		return sizeof (Gradient);
	}
	template<class A>
	void
	Gradient<2, A> :: show (String<>& string) const {
		show<String<> > (string);
	}
	...
	template<class A>
	Real
	Gradient<2, A> :: gradientMethod (const Real epsilon)
	{
		if (epsilon != 0) {
			epsilon_ = epsilon;
		}
	...
	template<class A>
	void
	Gradient<2, A> :: quickestDescent (const Real gradient[])
	{
	...
	template<class A>
	void
	Gradient<2, A> :: constShift (const Real gradient[])
	{
		Real vector [dim_];
		for (int i = 0; i < dim_; ++ i) {
	...
	template<class A>
	void
	Gradient<2, A> :: splittingShift (const Real gradient[])
	{
		const Real factor = 0.7;
...
#endif /*NSTD_INTERPOLATION_GRADIENT_2_CPP_*/
